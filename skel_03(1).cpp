...
/**DONE TODO: Now, lets read a message from the parent **/

if(read(parentToChildPipe[READ_END], fileNameRecv, MAX_FILE_NAME_LENGTH) < 0) //received file name
{
  perror("read");
  exit(-1);
}
...
cmdLine += fileNameRecv;
    /*DONE TODO: Open the pipe to the program (specified in cmdLine)
* using popen() and save the ouput into hashValue. See popen.cpp
    * for examples using popen
*/
FILE* hashOutput = popen(cmdLine.c_str(), "r");  //returns a ponter to the c-string representation of the string object's value
//to make sure that popen succeeded
  if(!hashOutput)
  {
    perror("popen");
    exit(-1);
  ...
  if(fread(hashValue, sizeof(char), sizeof(char) * HASH_VALUE_LENGTH, hashOutput) < 0)
{
    perror("fread");
    exit(-1);
}
...
/*DONE TODO: Send a string to the parent
   */
   //child sends computed hash value back to parent
...
{
/* I am the parent */
/** DONE TODO: close the unused ends of two pipes. **/

/*close write end of CP pipe to unblock parent's read() function and close read end of the PC pipe */
...
memset(hashValue, (char)NULL, HASH_VALUE_LENGTH);
// DONE TODO: Send the string to the child

//parent sends string fileName to child
